...
async function generate (prompt, style, prefix) {
  function handler (data, prefix) {
    switch (data.state) {
      ...
      case 'submitted':
        ...
        break
      case 'progress':
        const current = data.task.photo_url_list.length
        const max = styles.steps.get(style) + 1
  ...
  const res = await task(prompt, style, (data) => handler(data, prefix), { final, inter })
  if (!quiet && final) { console.log(`${prefix}Your results have been downloaded to the following files:`) } else if (!quiet) { console.log(`${prefix}Task finished, the results are available at the following addresses:`) }

  for (const inter of res.inter) { console.log(inter) }
...
(async () => {
  const prompt = argc._[0]
  const style = +argc._[1] || 3
